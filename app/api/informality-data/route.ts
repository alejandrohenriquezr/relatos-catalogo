...
export async function GET(request:NextRequest){
  ...
  if(cached&&request.nextUrl.searchParams.get("refresh")!=="1")return NextResponse.json({...JSON.parse(cached.payload_json),sources:JSON.parse(cached.source_url),cache:{status:"cached",checkedAt:cached.checked_at,updatedAt:cached.updated_at}},{headers:{"Cache-Control":"no-store"}});
  try{
    ...
    const signature=await combinedSha256(Object.values(files));

...
export const LABOR_STRUCTURE_ID =
  "INE.GOB.CL:DSD_ENE_MERCADO_LABORAL(2.0)";
...
const observation = (
  base: Pick<LaborRow, "DATASET" | "SEX" | "BREAKDOWN" | "CATEGORY" | "INDICATOR" | "UNIT_MEASURE" | "UNIT_MULT" | "EST_QUALITY">,
  point: AnyRecord,
  value: unknown,
): LaborRow | null => {
...
export function buildLaborRows(ene: AnyRecord, informality: AnyRecord) {
  ...
  for (const [sexLabel, points] of Object.entries(
    ...
    const sex = sexLabel === "Hombres" ? "M" : sexLabel === "Mujeres" ? "F" : "T";
    for (const point of points as AnyRecord[]) {
      ...
      for (const [indicator, raw] of Object.entries(values)) {
        ...
        const detail = typeof raw === "object" && raw ? raw as AnyRecord : {value: raw};
  ...
  for (const point of ene.seasonal ?? []) {
    const row = observation({
      DATASET: "ENE", SEX: "T", BREAKDOWN: "SEASONAL",
      CATEGORY: "TOTAL", INDICATOR: "UNEMPLOYMENT_RATE_SEASONALLY_ADJUSTED",
      UNIT_MEASURE: "PERCENT", UNIT_MULT: "0", EST_QUALITY: "F",
    }, point, point.value);
    if (row) rows.push(row);
  }
  for (const [name, series, breakdown] of [
    ...
    for (const point of series ?? []) for (const item of point.items ?? []) {
      for (const metric of ["change", "incidence"] as const) {
        const row = observation({
          DATASET: "ENE", SEX: "T", BREAKDOWN: breakdown,
          CATEGORY: safeCode(item.label), INDICATOR: `${name.toUpperCase()}_${metric.toUpperCase()}`,
          UNIT_MEASURE: "PERCENT", UNIT_MULT: "0", EST_QUALITY: "F",
        }, point, item[metric]);
        if (row) rows.push(row);
      }
  ...
  for (const point of ene.absentEmployment ?? []) {
    for (const [indicator, unit, mult] of [
      ["total", "PERSONS", "3"], ["present", "PERSONS", "3"],
      ["absent", "PERSONS", "3"], ["share", "PERCENT", "0"],
      ["change", "PERCENT", "0"], ["changePeople", "PERSONS", "0"],
      ["presentChange", "PERCENT", "0"],
    ]) {
      const row = observation({
        DATASET: "ENE", SEX: "T", BREAKDOWN: "EMPLOYMENT_PRESENCE",
        CATEGORY: "TOTAL", INDICATOR: safeCode(indicator),
        UNIT_MEASURE: unit, UNIT_MULT: mult, EST_QUALITY: quality(point.quality),
  ...
  for (const [snapshot, breakdown] of [
    ...
    const point = snapshot.year ? snapshot : {
      ...snapshot,
    ...
    for (const item of point.items) {
      const row = observation({
        DATASET: "ENE", SEX: "T", BREAKDOWN: breakdown,
        CATEGORY: safeCode(item.label), INDICATOR: "EMPLOYED",
        UNIT_MEASURE: "PERSONS", UNIT_MULT: "3", EST_QUALITY: quality(item.quality),
      }, point, item.value);
      if (row) rows.push(row);
    }
  }
  ...
  for (const point of informality.rates ?? []) {
    for (const [indicator, sex, unit, mult] of [
      ["formal", "T", "PERSONS", "3"], ["informal", "T", "PERSONS", "3"],
      ["menFormal", "M", "PERSONS", "3"], ["menInformal", "M", "PERSONS", "3"],
      ["womenFormal", "F", "PERSONS", "3"], ["womenInformal", "F", "PERSONS", "3"],
      ["rate", "T", "PERCENT", "0"], ["menRate", "M", "PERCENT", "0"],
      ["womenRate", "F", "PERCENT", "0"],
    ]) {
      const row = observation({
        DATASET: "INFORMALITY", SEX: sex, BREAKDOWN: "INDICATOR_MAIN",
        CATEGORY: "TOTAL", INDICATOR: safeCode(indicator),
        UNIT_MEASURE: unit, UNIT_MULT: mult, EST_QUALITY: quality(point[`${indicator}Quality`]),
      }, point, point[indicator]);
      if (row) rows.push(row);
    }
  }
  pushBreakdown(rows, informality.branches, "INFORMALITY", "ECONOMIC_ACTIVITY");
  pushBreakdown(rows, informality.categories, "INFORMALITY", "OCCUPATIONAL_CATEGORY");
  pushBreakdown(rows, informality.groups, "INFORMALITY", "OCCUPATION_GROUP");
  if (informality.hours?.items) for (const item of informality.hours.items) {
    for (const [indicator, unit, mult] of [
      ["rate", "PERCENT", "0"], ["informal", "PERSONS", "3"],
      ["share", "PERCENT", "0"], ["annual", "PERCENT", "0"],
    ]) {
      const row = observation({
        DATASET: "INFORMALITY", SEX: "T", BREAKDOWN: "USUAL_HOURS",
        CATEGORY: safeCode(item.label), INDICATOR: safeCode(indicator),
        UNIT_MEASURE: unit, UNIT_MULT: mult, EST_QUALITY: quality(item.note),
      }, informality.hours, item[indicator]);
      if (row) rows.push(row);
    }
  }
...
export function filterLaborRows(rows: LaborRow[], params: URLSearchParams) {
  ...
  const filters: [keyof LaborRow, string][] = [
    ["DATASET", "dataset"], ["REF_AREA", "ref_area"], ["SEX", "sex"],
    ["BREAKDOWN", "breakdown"], ["CATEGORY", "category"], ["INDICATOR", "indicator"],
  ];
  for (const [field, parameter] of filters) {
    const requested = params.get(parameter)?.split(",").map(safeCode);
    if (requested?.length) filtered = filtered.filter(row => requested.includes(row[field]));
  }
  const last = Number(params.get("last_n_periods"));
  if (Number.isInteger(last) && last > 0) {
    const periods = [...new Set(filtered.map(row => row.TIME_PERIOD))].sort().slice(-last);
...
export function laborRowsToCsv(rows: LaborRow[]) {
  return [
    LABOR_COLUMNS.join(","),
    ...rows.map(row => LABOR_COLUMNS.map(column => csv(row[column])).join(",")),
  ].join("\r\n");
}

export function laborSummary(rows: LaborRow[]) {
  const seriesKey = (row: LaborRow) =>
    [row.DATASET, row.REF_AREA, row.SEX, row.BREAKDOWN, row.CATEGORY, row.INDICATOR].join(".");
